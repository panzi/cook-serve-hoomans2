...
if platform.system() == 'Linux':
	...
	def find_archive():
		path = pjoin(os.getenv("HOME"), "Library/Application Support/Steam/SteamApps/common/CookServeDelicious2/Cook Serve Delicious 2.app/Contents/Resources/game.ios")
		if isfile(path):
			return path
		...
		for key, subkey, valkey in reg_keys:
			try:
				...
				try:
					...
					if valtype == REG_SZ:
						return pjoin(value, "steamapps\\common\\CookServeDelicious2\\data.win")
				finally:
					CloseKey(hkey)
			except FileNotFoundError:
				pass
